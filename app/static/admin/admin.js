...
// Navigation Logic
        ...
        function parseTrendDate(dateStr) {
            ...
            if (!dateStr.includes('T')) {
                if (dateStr.length === 7) return new Date(dateStr + '-01T00:00:00');
                if (dateStr.length === 10) return new Date(dateStr + 'T00:00:00');
                if (dateStr.length === 13) return new Date(dateStr.replace(' ', 'T') + ':00:00');
                if (dateStr.length === 16) return new Date(dateStr.replace(' ', 'T') + ':00');
            }
