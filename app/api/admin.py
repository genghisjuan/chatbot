...
from app.services.analytics import EscalationLog, ExpenseLog, FeedbackLog, QueryLog

# Use zoneinfo for timezone handling
try:
    from zoneinfo import ZoneInfo
except ImportError:  # pragma: no cover
    from backports.zoneinfo import ZoneInfo  # type: ignore[assignment]  # Fallback
...
def _ensure_aware_utc(ts: datetime) -> datetime:
    ...
    NOTE: Preserves the existing behavior for SQLite-like returns:
    if `tzinfo` is missing, we assume UTC.
    """
...
def get_summary(
    ...
    try:
        ...
        queries = (
            db.query(QueryLog)
            .filter(QueryLog.timestamp >= start_utc, QueryLog.timestamp <= end_utc)
            .count()
        )
        ...
        _avg_qpc = (queries / conversations) if conversations > 0 else 0  # kept for parity
...
def get_trends(
    ...
    # 1. Fetch RAW data
    query_logs = (
        db.query(QueryLog.timestamp, QueryLog.is_initial)
        .filter(QueryLog.timestamp >= start_utc, QueryLog.timestamp <= end_utc)
        .all()
    )
    ...
    fb_timeline: Dict[str, Dict[str, int]] = {}
    ...
    for q_ts, is_init in query_logs:
        ...
        q_ts_utc = _ensure_aware_utc(q_ts)
        ts_est = q_ts_utc.astimezone(EST)
    ...
    # --------------------------------------------------------------------------
    feedback_logs = (
        db.query(FeedbackLog.timestamp, FeedbackLog.rating)
        .filter(FeedbackLog.timestamp >= start_utc, FeedbackLog.timestamp <= end_utc)
        .all()
    )
    ...
    for f_ts, rating in feedback_logs:
        ...
        f_ts_utc = _ensure_aware_utc(f_ts)
        ts_est = f_ts_utc.astimezone(EST)
        bucket_key = _truncate_date(ts_est, granularity)

        if bucket_key not in fb_timeline:
            fb_timeline[bucket_key] = {"pos": 0, "neg": 0}

        # Normalize rating value (SQLite may return strings even when stored as int)
    ...
    ds_pos_count = [fb_timeline[k]["pos"] for k in fb_sorted_keys]
    ds_neg_count = [fb_timeline[k]["neg"] for k in fb_sorted_keys]
...
def get_spend(
    ...
    expense_stats = (
        db.query(func.sum(ExpenseLog.amount))
        .filter(ExpenseLog.timestamp >= start_utc, ExpenseLog.timestamp <= end_utc)
        .scalar()
    )
...
def _send_alert_email(subject: str, body: str) -> None:
    ...
    msg = MIMEText(body)
    msg["Subject"] = f"[SUPPORT ALERT] {subject}"
    msg["From"] = sender
    msg["To"] = recipient
    ...
    if total > 5:  # Minimum sample size to avoid noise
        ...
        if rate > 20:
            background_tasks.add_task(
                ...
                (
                    f"Alert: Negative feedback rate is {rate:.1f}% in the last hour "
                    f"(Threshold: 20%). Total feedback: {total}."
                ),
            )
